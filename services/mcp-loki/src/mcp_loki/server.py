...
from mcp_loki import __version__
from mcp_loki.client import LokiError
from mcp_loki.tools import REGISTRY

log = structlog.get_logger(__name__)

app = FastAPI(title="mcp-loki", version=__version__)
...
async def health() -> dict[str, str]:
    return {"status": "ok", "server": "mcp-loki", "version": __version__}
...
async def invoke(req: InvokeRequest) -> InvokeResponse:
    ...
    try:
        ...
        log.error("loki_api_error", tool=req.tool, status=e.status)
        raise HTTPException(
            status_code=502,
