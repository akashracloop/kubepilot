"""Datadog MCP server — same REST-flavored MCP shape as mcp-prom/mcp-loki.

- /mcp/health   liveness
- /mcp/tools    tool descriptors (read-only query/search tools)
- /mcp/invoke   tool invocation → curated capability shapes
"""
...
from mcp_datadog import __version__
from mcp_datadog.client import DatadogError
from mcp_datadog.tools import REGISTRY

log = structlog.get_logger(__name__)

app = FastAPI(title="mcp-datadog", version=__version__)
...
async def health() -> dict[str, str]:
    return {"status": "ok", "server": "mcp-datadog", "version": __version__}
...
async def invoke(req: InvokeRequest) -> InvokeResponse:
    ...
    try:
        ...
        log.error("datadog_api_error", tool=req.tool, status=e.status)
        raise HTTPException(
            status_code=502, detail={"upstream_status": e.status, "tool": req.tool}
        ) from e
