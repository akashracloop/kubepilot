...
Same REST-flavored MCP shape as mcp-k8s/mcp-prom (see docs/ARCHITECTURE.md §6.1):
  - /mcp/health   liveness
...
from mcp_tempo import __version__
from mcp_tempo.client import TempoError
from mcp_tempo.tools import REGISTRY

log = structlog.get_logger(__name__)

app = FastAPI(title="mcp-tempo", version=__version__)
...
async def health() -> dict[str, str]:
    return {"status": "ok", "server": "mcp-tempo", "version": __version__}
...
async def invoke(req: InvokeRequest) -> InvokeResponse:
    ...
    try:
        ...
        log.error("tempo_api_error", tool=req.tool, status=e.status)
        raise HTTPException(
            status_code=502,
