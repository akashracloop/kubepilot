...
from mcp_ci import __version__
from mcp_ci.client import CIError
from mcp_ci.tools import REGISTRY

log = structlog.get_logger(__name__)

app = FastAPI(title="mcp-ci", version=__version__)
...
async def health() -> dict[str, str]:
    return {"status": "ok", "server": "mcp-ci", "version": __version__}
...
async def invoke(req: InvokeRequest) -> InvokeResponse:
    ...
    try:
        ...
        log.error("ci_api_error", tool=req.tool, status=e.status)
        raise HTTPException(
            status_code=502,
